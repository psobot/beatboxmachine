...
import time, sys, wave, mimetypes, config
...
class Remixer( Thread ):
    ...
    def mixwav( self, filename ):
        ...
        if self.encoded is 1:
            rename( "%s%s%s.wav" % ( self.tempdir, self.uid, 0 ), filename )
            return
    ...
    def getTag( self ):
        ...
        try:
            ...
            if hasattr(self.mt, 'info'):
                ...
                tag['bitrate'] = ( wav._file.getsize() / tag['length'] ) / 1.25  # value in kilobits
                wav.close()
                del wav
    ...
    def processArt( self ):
        ...
        try:
            ...
            if imgmime and imgdata:
                ...
                artwork.resize( config.thumbnail_size, Image.ANTIALIAS ).convert( "RGB" ).save( self.thumbpath )
                ...
                raise Exception( "No artwork data found!" )
        except:
            self.artprocessed = False
            return False
    ...
    def updateTags( self, titleSuffix='' ):
        ...
        """
        try:
            self.tag['new_title'] = "%s%s" % ( self.tag['title'] if 'title' in self.tag else '[untitled]', titleSuffix )
